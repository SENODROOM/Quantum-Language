...
Token Lexer::readNumber() {
    ...
    if (current() == '0' && (peek() == 'x' || peek() == 'X')) {
        ...
        while (pos < src.size() && std::isxdigit(current())) num += advance();
    } else {
...
std::vector<Token> Lexer::tokenize() {
    ...
    while (pos < src.size()) {
        ...
        if (c == '"' || c == '\'') { tokens.push_back(readString(c)); continue; }
        ...
        switch (c) {
            case '+':
                if (current() == '=') { advance(); tokens.emplace_back(TokenType::PLUS_ASSIGN, "+=", startLine, startCol); }
            ...
            case '-':
                ...
                else if (current() == '=') { advance(); tokens.emplace_back(TokenType::MINUS_ASSIGN, "-=", startLine, startCol); }
                else tokens.emplace_back(TokenType::MINUS, "-", startLine, startCol);
                break;
            case '*':
                if (current() == '*') { advance(); tokens.emplace_back(TokenType::POWER, "**", startLine, startCol); }
                else if (current() == '=') { advance(); tokens.emplace_back(TokenType::STAR_ASSIGN, "*=", startLine, startCol); }
                else tokens.emplace_back(TokenType::STAR, "*", startLine, startCol);
                break;
            case '/':
                if (current() == '/') { skipComment(); }
                else if (current() == '=') { advance(); tokens.emplace_back(TokenType::SLASH_ASSIGN, "/=", startLine, startCol); }
            ...
            case '=':
                if (current() == '=') { advance(); tokens.emplace_back(TokenType::EQ, "==", startLine, startCol); }
            ...
            case '!':
                ...
                break;
            case '<':
                if (current() == '=') { advance(); tokens.emplace_back(TokenType::LTE, "<=", startLine, startCol); }
                else if (current() == '<') { advance(); tokens.emplace_back(TokenType::LSHIFT, "<<", startLine, startCol); }
                else tokens.emplace_back(TokenType::LT, "<", startLine, startCol);
                break;
            case '>':
                if (current() == '=') { advance(); tokens.emplace_back(TokenType::GTE, ">=", startLine, startCol); }
                else if (current() == '>') { advance(); tokens.emplace_back(TokenType::RSHIFT, ">>", startLine, startCol); }
                else tokens.emplace_back(TokenType::GT, ">", startLine, startCol);
                break;
            case '&': tokens.emplace_back(TokenType::BIT_AND, "&", startLine, startCol); break;
            case '|': tokens.emplace_back(TokenType::BIT_OR, "|", startLine, startCol); break;
            case '^': tokens.emplace_back(TokenType::BIT_XOR, "^", startLine, startCol); break;
            case '~': tokens.emplace_back(TokenType::BIT_NOT, "~", startLine, startCol); break;
            ...
            case '.': tokens.emplace_back(TokenType::DOT, ".", startLine, startCol); break;
            default:
